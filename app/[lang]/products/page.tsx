...
async function layDanhSachDanhMuc(ngonNguHienTai:string){
  const all:any[]=await sanityClient.fetch(`*[_type == "service" && defined(slug.current) && !(_id in path("drafts.**"))] {_id,title,language,_translationKey}`)
  const groups:Record<string,any[]>={}
  all.forEach((item)=>{const key=item._translationKey||item._id;if(!groups[key])groups[key]=[];groups[key].push(item)})
  return Object.values(groups).map((group)=>group.find((v)=>v.language===ngonNguHienTai)||group.find((v)=>v.language==='en')||group.find((v)=>v.language==='vi')||group[0])
}
...
export default async function ProductsListPage({params}:{params:Promise<{lang:string}>}){
  const {lang}=await params
  const [dictionary,danhSachSanPham,danhSachDanhMuc,siteName]=await Promise.all([getDictionary(lang),layDanhSachSanPham(lang),layDanhSachDanhMuc(lang),getSiteName()])
