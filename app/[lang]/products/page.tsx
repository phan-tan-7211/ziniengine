...
import { getProductServiceCategories } from "@/lib/service-catalog"
...
export default async function ProductsListPage({params}:{params:Promise<{lang:string}>}){
  const {lang}=await params
  const [dictionary,danhSachSanPham,danhSachDanhMuc,siteName]=await Promise.all([getDictionary(lang),layDanhSachSanPham(lang),getProductServiceCategories(lang),getSiteName()])
