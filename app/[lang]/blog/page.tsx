...
async function getBlogPosts(lang: string) {
  const raw:any[] = await sanityClient.fetch(`*[_type == "blogPost" && defined(slug.current) && !(_id in path("drafts.**"))] { _id,_translationKey,title,"slug":slug.current,language,excerpt,"mainImage":mainImage.asset->{url},publishedAt,author,readTime,"category":category->title }`)
  const groups:Record<string,any[]> = {}
  raw.forEach((post)=>{ const key=post._translationKey||post._id; if(!groups[key]) groups[key]=[]; groups[key].push(post) })
...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }) {
  ...
  return {title:{absolute:title},description,alternates:{canonical:`/${lang}/blog`,languages:{"vi-VN":"/vi/blog","en-US":"/en/blog","ja-JP":"/jp/blog","ko-KR":"/kr/blog","zh-CN":"/cn/blog","x-default":"/vi/blog"}},openGraph:{title,description,url:`/${lang}/blog`,siteName,type:"website"},twitter:{card:"summary_large_image",title,description}}
}
